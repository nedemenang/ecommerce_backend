...
export const sendWelcomeEmail = (recipientName, recipientMail, subject, type, confirmCode) => {
  return new Mail(type)
    .to(recipientMail)
    .subject(subject)
    .data({
      name: recipientName,
      url: `${process.env.url}/customers/verify/${confirmCode}`,
    })
    .send();
};
...
export const sendForgotPasswordMail = async (recipientName, recipientMail, subject, type, url) => {
  return new Mail(type)
    .to(recipientMail)
    .subject(subject)
    .data({
